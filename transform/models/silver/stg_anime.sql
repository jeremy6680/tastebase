-- =============================================================================
-- stg_anime.sql — Silver layer: anime domain
-- =============================================================================
--
-- Sources:
--   - raw_moviebuddy  (content_type = 'TV Show' AND genres LIKE '%Anime%')
--   - raw_trakt       (endpoint = 'watched_shows', anime detection via title
--                      cross-reference — genre data not in watched_shows payload)
--
-- Note on Trakt anime detection: Trakt's watched_shows payload does not
-- include genre information. We rely on MovieBuddy as the primary anime
-- source. Trakt shows that match a MovieBuddy anime entry by imdb_id
-- are merged via deduplication. Trakt-only anime shows will appear in
-- stg_series until genre enrichment is added in a future phase.
--
...
-- Trakt watched shows that match a known anime by IMDB ID.
-- This covers anime watched on Trakt that also exist in MovieBuddy.
-- Trakt-only anime (no IMDB ID match) remain in stg_series for now.
trakt_anime_ids AS (
    SELECT DISTINCT imdb_id
    FROM moviebuddy_anime
...
trakt_shows AS (
    ...
    WHERE endpoint = 'watched_shows'
      ...
      -- Only include Trakt shows that match a known MovieBuddy anime
      AND REGEXP_EXTRACT(payload, '''imdb'':\s*''(tt\d+)''', 1) IN (
          SELECT imdb_id FROM trakt_anime_ids
      )
),
...
all_anime AS (
    SELECT * FROM moviebuddy_anime
    UNION ALL
    SELECT * FROM trakt_anime_rated
),
